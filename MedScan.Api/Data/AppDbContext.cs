...
public class AppDbContext : IdentityDbContext<ApplicationUser> {
    ...
    public async Task SeedHomePharmacyItemsAsync()
    {
        var profileIds = await Profiles
            .AsNoTracking()
            .Select(p => p.Id)
            .ToListAsync();
        ...
        var seedDefs = new[]
        {
            new { Barcode = "3800010640916", ExpiresOn = new DateOnly(2030, 7, 31), Quantity = 1 },
            new { Barcode = "3800010646529", ExpiresOn = new DateOnly(2030, 3, 31), Quantity = 1 },
            new { Barcode = "5055565732748", ExpiresOn = new DateOnly(2030, 3, 31), Quantity = 1 },
            new { Barcode = "5010123729189", ExpiresOn = new DateOnly(2027, 6, 30), Quantity = 1 },
            new { Barcode = "4013054029832", ExpiresOn = new DateOnly(2028, 11, 30), Quantity = 1 },
            new { Barcode = "04030855234233", ExpiresOn = new DateOnly(2030, 2, 28), Quantity = 1 },
            new { Barcode = "05290931027022", ExpiresOn = new DateOnly(2029, 10, 31), Quantity = 1 },
            new { Barcode = "07613421029043", ExpiresOn = new DateOnly(2027, 6, 30), Quantity = 1 },
            new { Barcode = "03582910055372", ExpiresOn = new DateOnly(2027, 12, 31), Quantity = 1 },
            new { Barcode = "4742041002907", ExpiresOn = new DateOnly(2026, 7, 31), Quantity = 1 },
            new { Barcode = "5000158104273", ExpiresOn = new DateOnly(2027, 1, 31), Quantity = 1 }
        };

        var barcodes = seedDefs.Select(x => x.Barcode).Distinct().ToList();
        var medicationIdsByBarcode = await Medications
            .AsNoTracking()
            .Where(m => barcodes.Contains(m.Barcode))
            .ToDictionaryAsync(m => m.Barcode, m => m.Id);

        if (medicationIdsByBarcode.Count == 0)
        {
            return;
        }

        var now = DateTime.UtcNow;
        var toAdd = new List<HomePharmacyItem>();

        foreach (var profileId in profileIds)
        {
            var existingMedicationIds = await HomePharmacyItems
                .AsNoTracking()
            ...
            {
                if (!medicationIdsByBarcode.TryGetValue(def.Barcode, out var medicationId))
                {
                    continue;
                }
                ...
                toAdd.Add(new HomePharmacyItem
                {
                    ProfileId = profileId,
                ...
                existingMedicationIds.Add(medicationId);
            }
        }
        ...
        await HomePharmacyItems.AddRangeAsync(toAdd);
        await SaveChangesAsync();
    }
